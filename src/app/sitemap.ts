...
async function getPopularMovies() {
  try {
    // Fetch multiple pages to get 2000 movies (100 pages * 20 per page)
    const pages = await Promise.all(
      Array.from({ length: 100 }, (_, i) =>
        fetch(`${TMDB_CONFIG.BASE_URL}/movie/popular?page=${i + 1}`, {
          headers: TMDB_CONFIG.headers,
...
async function getPopularTVShows() {
  try {
    // Fetch multiple pages to get 2000 TV shows (100 pages * 20 per page)
    const pages = await Promise.all(
      Array.from({ length: 100 }, (_, i) =>
        fetch(`${TMDB_CONFIG.BASE_URL}/tv/popular?page=${i + 1}`, {
          headers: TMDB_CONFIG.headers,
          next: { revalidate: 86400 },
        }).then((res) => res.json())
      )
...
export default async function sitemap(): Promise<MetadataRoute.Sitemap> {
  ...
  try {
    ...
    const tvPages = popularTVShows.results
      .slice(0, 2000)
      .map((show: TVShow) => ({
