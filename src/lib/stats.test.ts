...
describe("formatWatchTime", () => {
  ...
  it("reports under an hour in minutes", () => {
    // One ticked episode is the first number this page ever shows, and rounding
    // it into hours made it "0 hours".
    expect(formatWatchTime(22)).toBe("22 minutes");
  ...
  it("says nothing grand about nothing", () => {
    ...
    expect(formatWatchTime(-5)).toBe("no time yet");
    expect(formatWatchTime(Number.NaN)).toBe("no time yet");
  });
});
