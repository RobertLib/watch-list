...
describe("buildCalendarIcs", () => {
  ...
  // The query-string form is the only one this app serves: a static export has
  // no dynamic routes, so `/tv/breaking-bad-1396` is a 404 and `/tv?id=…` is the
  // page. Asserted here because the export used to emit the path form.
  it("links each event back to its page", () => {
    const output = lines(buildCalendarIcs([episode], OPTIONS));

    expect(output).toContain(
      "URL:https://www.watch-list.me/tv?id=breaking-bad-1396",
    );
  });
