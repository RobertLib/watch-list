...
describe("provider id storage round-trip", () => {
  it("survives the trip through the stored value", () => {
    const ids = [8, 337, 1899];
    expect(parseStoredProviderIds(providerIdsToStoredValue(ids))).toEqual(
      ids,
    );
  });

  it("treats an absent value as no selection rather than as an error", () => {
    expect(parseStoredProviderIds("")).toEqual([]);
  });

  it("skips entries hand-edited storage might carry", () => {
    expect(parseStoredProviderIds("8,,abc,-2,0,337")).toEqual([8, 337]);
  });
});
