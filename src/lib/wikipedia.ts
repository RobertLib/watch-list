...
//
// Called from the browser, which changes two things. `origin=*` is what makes
// the API answer with a permissive CORS header – without it the request is made
// and then thrown away unread. And the User-Agent header is gone: browsers
// forbid setting it, and a rejected header fails the whole request.
//
// Both calls go through `fetchWithRetry` for its deadline rather than for its
// retry. A bare `fetch` has no timeout at all: a request Wikipedia never answers
// is also never abandoned, and the insights panel below a film would sit on its
// spinner for as long as the tab stayed open. Ten seconds and then null, which
// every caller here already handles as "no article".
// ──────────────────────────────────────────────────────────────────────────────
