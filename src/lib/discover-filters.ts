...
export function discoverFiltersToFilterOptions(
  ...
  //
  // Both halves have to read the same clock. They did not: the year came from
  // `getFullYear()` – local – and the ceiling from `toISOString()` – UTC. On the
  // 1st of January east of Greenwich that pair is a year apart, so the filter
  // recognised the new year and then capped it at the 31st of December, which
  // matches nothing.
  const today = todayLocal();
  if (filters.year === today.slice(0, 4)) {
    if (type === "movie") {
...
// ──────────────────────────────────────────────────────────────────────────────
// The TMDB query boundary
//
// `parseDiscoverFilters` above covers everything that arrives through the URL as
// the filter bar writes it. This second pass re-derives a `FilterOptions` from
// scratch instead of trusting the shape, because the same object also reaches
// TMDB from hand-edited query strings and from the mood definitions – and what
// it becomes there is a request we would rather not make on someone's behalf.
// ──────────────────────────────────────────────────────────────────────────────
...
export function sanitizeFilterOptions(
  ...
  // from the filter bar, but they pass through the same sanitizer regardless –
  // one path through the validator is one place to look when it is wrong.
