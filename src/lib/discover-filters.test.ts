...
describe("discoverFiltersToFilterOptions", () => {
  /**
   * The current-year filter caps its upper bound at today so that titles TMDB
   * has not released yet stay out of a listing for a year that is still running.
   *
   * Both halves have to read the same clock, and they did not: the year came
   * from `getFullYear()` – local – and the ceiling from `toISOString()` – UTC.
   * Those disagree for part of every day, and across New Year they disagree by
   * a whole year: in the Americas on the evening of the 31st of December the
   * filter matched "this year" and then capped it in the *next* one, and east of
   * Greenwich in the small hours of the 1st of January it matched the new year
   * and capped it at the 31st of December – a window matching nothing at all.
   *
   * Stated as an invariant rather than against a fixed instant so that it holds
   * in both zones CI runs the suite in.
   */
  it("caps the current year inside that same year", () => {
    const now = new Date();
    const localYear = String(now.getFullYear());

    const movie = discoverFiltersToFilterOptions(
      { ...EMPTY_DISCOVER_FILTERS, year: localYear },
      "movie",
    );
    ...
    expect(movie.primaryReleaseDateLte).toMatch(
      new RegExp(`^${localYear}-\\d{2}-\\d{2}$`),
    );
    expect(tv.firstAirDateLte).toMatch(
      new RegExp(`^${localYear}-\\d{2}-\\d{2}$`),
    );
  });

  it("leaves a past year uncapped – the whole year has happened", () => {
    const past = String(new Date().getFullYear() - 3);

    expect(
      discoverFiltersToFilterOptions(
        { ...EMPTY_DISCOVER_FILTERS, year: past },
        "movie",
