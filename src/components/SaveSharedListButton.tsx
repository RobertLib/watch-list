...
export function SaveSharedListButton({ items }: { items: MediaItem[] }) {
  ...
  function saveAll() {
    const results = missing.map((item) => addItem(item));
    const added = results.filter((result) => result === "ok").length;
    // One refused write means the rest were refused too – the browser is out of
    // room, not confused about one title.
    const refused = results.some((result) => result === "failed");
