...
import { getCachedMovieWatchProviders } from "@/lib/tmdb-cache";
...
async function MovieWatchProvidersContent({
  movieId,
  ...
  if (!watchProviders && movieId) {
    const currentRegion = await getRegion();
    const regionCode = getRegionCode(currentRegion);
    watchProviders = await getCachedMovieWatchProviders(movieId, regionCode);
  }
