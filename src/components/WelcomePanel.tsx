...
export function WelcomePanel({ hasUserSettings }: WelcomePanelProps) {
  ...
  useEffect(() => {
    ...
    const panelDismissed = localStorage.getItem("welcome-panel-dismissed");

    // Show panel only if:
    // 1. User hasn't dismissed it
    // 2. User doesn't have custom settings (region or watch provider filter)
    if (!panelDismissed && !hasUserSettings) {
      setIsVisible(true);
    }
  }, [hasUserSettings]);
  ...
  return (
    <div
      className="bg-gradient-to-r from-blue-900/20 to-purple-900/20 border border-blue-800/30 rounded-lg p-6 mb-8 relative"
      role="banner"
    ...
    >
      ...
      <div className="sm:pr-8">
        <div className="flex items-center gap-3 mb-4">
          <div
            className="h-12 w-12 bg-gradient-to-br from-red-500 to-red-600 rounded-full flex items-center justify-center"
            role="img"
