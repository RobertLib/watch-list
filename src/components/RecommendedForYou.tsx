...
import { getWatchlistRecommendations } from "@/lib/api";
import type { RecommendationsResult } from "@/lib/recommendations";
import type { MediaType } from "@/types/tmdb";

// Mirrors the cap the resolver applies – anything beyond it is discarded there.
...
export function RecommendedForYou() {
  ...
  // The picks on screen, together with the two lists they were built from.
  // Holding both together is what lets "still fetching" be derived below rather
  // than tracked as a flag the effect has to raise on its way in.
  const [loaded, setLoaded] = useState<{
    seeds: readonly [typeof watchlistSeeds, typeof watchedSeeds] | null;
    result: RecommendationsResult;
  }>({ seeds: null, result: EMPTY_RESULT });

  const result = loaded.result;
  const isFetching =
    loaded.seeds?.[0] !== watchlistSeeds || loaded.seeds?.[1] !== watchedSeeds;

  useEffect(() => {
    // Nothing saved yet – the carousel renders nothing either way, so there is
    // no request to make.
    if (isLoading || seedCount === 0) return;

    let isCurrent = true;
    const seeds = [watchlistSeeds, watchedSeeds] as const;
    ...
    const timer = setTimeout(async () => {
      try {
        ...
        if (isCurrent) setLoaded({ seeds, result: recommendations });
      } catch (error) {
        console.error("Error loading recommendations:", error);
        if (isCurrent) setLoaded({ seeds, result: EMPTY_RESULT });
      }
    }, REFRESH_DELAY_MS);
