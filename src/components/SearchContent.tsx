...
import { countTitleResults } from "@/lib/search-totals";
...
function SearchResults() {
  ...
  const page = sanitizePage(Number(searchParams.get("page")) || 1);
  ...
  const { data, isLoading } = useAsyncData(async () => {
    ...
    const [mediaResult, peopleResult] = await Promise.allSettled([
      tmdbDiscoverApi.searchMulti(query, page),
      // Asked for on every page, shown only on the first – by page three nobody
      // is looking for an actor any more.
      //
      // It is still fetched past page one because its `total_results` is what
      // turns the multi-search total into a count of titles: `/search/multi`
      // counts people among its results and the grid shows none of them. The
      // URL is identical on every page, so after the first this is answered
      // from the transport cache rather than from the network.
      tmdbDiscoverApi.searchPerson(query, 1),
    ]);

    return {
      media: mediaResult.status === "fulfilled" ? mediaResult.value : null,
      people: peopleResult.status === "fulfilled" ? peopleResult.value : null,
      failed: mediaResult.status === "rejected",
    };
  }, [query, page, settingsKey]);

  if (!query) return <EmptyQuery />;
  if (isLoading || !data) return <SearchSkeleton />;

  const { media, people, failed } = data;
  const items = media?.results ?? [];
  const persons = page === 1 ? (people?.results ?? []) : [];
  const totalPages = Math.min(media?.total_pages ?? 1, 500);
  const totalResults = media?.total_results ?? 0;

  // The multi-search total counts people too, and the grid below shows none of
  // them – see `countTitleResults` for why the people lookup is what corrects it.
  const titleResults = countTitleResults(totalResults, people?.total_results);
    ...
    <div className="container mx-auto px-4 py-8">
      <div className="mb-8">
        ...
        {failed ? (
          ...
          <p className="text-gray-400">
            {titleResults > 0
              ? `${titleResults.toLocaleString("en-US")} title${
                  titleResults === 1 ? "" : "s"
