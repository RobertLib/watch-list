...
export function LanguageSupport({ translations }: LanguageSupportProps) {
  ...
  // Until the page is hydrated the region is still the default, and highlighting
  // the wrong country's languages is worse than highlighting none.
  const loading = !useHydrated();

  // Get all unique languages from translations
  const allLanguages = translations.translations.reduce(
    (acc, translation) => {
