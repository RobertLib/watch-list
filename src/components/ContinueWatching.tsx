...
export function ContinueWatching() {
  ...
  // The episodes on screen, together with the ticks they were resolved from.
  // Keeping the two in one place is what lets "still fetching" be derived below
  // instead of tracked as a second flag the effect has to set on the way in.
  const [loaded, setLoaded] = useState<{
    seeds: typeof seeds | null;
    episodes: UpNextEpisode[];
  }>({ seeds: null, episodes: [] });

  const episodes = loaded.episodes;
  const isFetching = loaded.seeds !== seeds;

  useEffect(() => {
    // Nothing tracked yet, or nothing in progress – the row renders nothing
    // either way, so there is no request to make.
    ...
    const timer = setTimeout(async () => {
      try {
        ...
        if (isCurrent) setLoaded({ seeds, episodes: next });
      } catch (error) {
        console.error("Error loading continue watching:", error);
        if (isCurrent) setLoaded({ seeds, episodes: [] });
      }
    }, REFRESH_DELAY_MS);
