...
async function PageHeroAsync({
  ...
  try {
    ...
    if (mediaType === "movie") {
      ...
      // For "all" use random movie or TV show
      const useMovies = Math.random() > 0.5;
      backgroundItem = useMovies
        ? staticBackgrounds.movie
        : staticBackgrounds.tv;
    }
    ...
    console.error("Error loading page hero background:", error);
    return <PageHeroSkeleton title={title} description={description} />;
  }
}
