...
export function HeroButtons({ featuredItem }: HeroButtonsProps) {
  ...
  const handlePlayClick = async () => {
    ...
    await openVideo(featuredItem.id, mediaType);
  };

  const handleMoreInfoClick = () => {
    const slug = createSlug(featuredItem.title, featuredItem.id);
    const path =
    ...
    <>
      ...
      <div className="flex flex-col sm:flex-row gap-3 sm:gap-4 pt-2 sm:pt-4">
        <button
          ...
          aria-label={`Play trailer for ${featuredItem.title}`}
          aria-busy={isLoading}
        >
        ...
        <button
          ...
          aria-label={`View more information about ${featuredItem.title}`}
        >
