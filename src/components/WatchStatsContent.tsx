...
import { getWatchStatsFacts } from "@/lib/api";
...
export function WatchStatsContent() {
  ...
  // so reading it during render would make the prerendered HTML and the first
  // client render disagree. Same arrangement as the watchlist and watched stores.
  useEffect(() => {
    /* eslint-disable-next-line react-hooks/set-state-in-effect -- hydrating from a browser-only store, see above */
    setGoal(getGoal());
  }, []);
  ...
  useEffect(() => {
    ...
    // Nothing watched and nothing ticked, so there are no runtimes to resolve.
    // The spinner keys off the same emptiness, so the empty state renders
    // without a load ever settling.
  ...
  if (isLoading || (!hasLoaded && refs.length > 0)) {
    return (
      <div className="py-16">
        <LoadingSpinner />
      </div>
