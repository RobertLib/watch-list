...
import { getTitlesByRefs } from "@/lib/api";
...
export function RatedTitles() {
  ...
  useEffect(() => {
    // Nothing rated, so there are no cards to fetch. The spinner below keys off
    // the same emptiness, so the empty state renders without a load settling.
    if (lookupKey === "") return;
      ...
      <ul className="space-y-2">
        {sorted.map((ref) => {
          ...
          return (
            ...
            >
              <Link
                href={mediaHref(ref.mediaType, createSlug(title, ref.id))}
                prefetch={false}
