...
async function SimilarTVShowsContent({ tvId }: SimilarTVShowsProps) {
  const similar = await tmdbApi.getSimilarTVShows(tvId);
    ...
    <section>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-4">
        {similar.results.slice(0, 12).map((show) => {
          ...
          return (
            <Link key={show.id} href={`/tv/${slug}`} className="block">
...
export function SimilarTVShows({ tvId }: SimilarTVShowsProps) {
  return (
    <Suspense fallback={<LoadingSimilarTVShows />}>
      <SimilarTVShowsContent tvId={tvId} />
    </Suspense>
  );
}
