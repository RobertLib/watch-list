...
async function TVDetailsContent({ tvId }: TVDetailsProps) {
  const details = await tmdbApi.getTVShowDetails(tvId);
...
export function TVDetails({ tvId }: TVDetailsProps) {
  return (
    <Suspense fallback={<LoadingTVDetails />}>
      <TVDetailsContent tvId={tvId} />
    </Suspense>
  );
}
